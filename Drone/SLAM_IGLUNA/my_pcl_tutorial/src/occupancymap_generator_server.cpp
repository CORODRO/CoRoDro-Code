...
else {twofiveD_depth[j*width+i].push_back(search_point.z);
}
}
...
//In PGM file, pixel are written from top left corner to bottom right corner, so you need
//to print on the file the cells in row order, beginning with the row of maximum index and end with the first row of the gridlist
//As a pgm file 
...
std::string mapdatafile_discret = mapname + "_discretized.pgm";
	...
	for(unsigned int i=0;i!=occupancygriddiscret.info.height;i++){
        for(unsigned int j=0;j!=occupancygriddiscret.info.width;j++){
	unsigned int x=j+(occupancygriddiscret.info.height-1-i)*occupancygriddiscret.info.width;
          if (occupancygriddiscret.data[x] >= 0 && occupancygriddiscret.data[x] <= threshold_free) { // Free is 254 : white
            fputc(254, out_discret);
          } else if (occupancygriddiscret.data[x] >= threshold_occupied) { // Occupied is black : 000
...
std::string mapdatafile = mapname + ".pgm";
      ...
      fclose(out);
//As a txt file 
std::string maptxtdatafile = mapname + "forplanning.txt";
